...
function mixColumns(state) {
  ...
  for (let c = 0; c < 4; c++) {
    ...
    out[i]   = mul(2,a[0]) ^ mul(3,a[1]) ^ a[2] ^ a[3];
    out[i+1] = a[0] ^ mul(2,a[1]) ^ mul(3,a[2]) ^ a[3];
    out[i+2] = a[0] ^ a[1] ^ mul(2,a[2]) ^ mul(3,a[3]);
    out[i+3] = mul(3,a[0]) ^ a[1] ^ a[2] ^ mul(2,a[3]);
  }
  return out;
}
...
function invMixColumns(state) {
  ...
  for (let c = 0; c < 4; c++) {
    const i = c*4;
    const a = state.slice(i, i+4);
    out[i]   = mul(0x0e,a[0]) ^ mul(0x0b,a[1]) ^ mul(0x0d,a[2]) ^ mul(0x09,a[3]);
    out[i+1] = mul(0x09,a[0]) ^ mul(0x0e,a[1]) ^ mul(0x0b,a[2]) ^ mul(0x0d,a[3]);
    out[i+2] = mul(0x0d,a[0]) ^ mul(0x09,a[1]) ^ mul(0x0e,a[2]) ^ mul(0x0b,a[3]);
    out[i+3] = mul(0x0b,a[0]) ^ mul(0x0d,a[1]) ^ mul(0x09,a[2]) ^ mul(0x0e,a[3]);
  }
  return out;
}
...
function expandKey(key) {
  ...
  const roundKeys = [];
  for (let i=0;i<11;i++) roundKeys.push(w.slice(i*16,(i+1)*16));
  return roundKeys;
}
...
export const hexToBytes = (hex) => {
  ...
  for (let i=0;i<hex.length/2;i++) arr[i]=parseInt(hex.substr(i*2,2),16);
  return arr;
};
